 ...
 * @param {object} el [canvas容器,必须是DOM对象]
 * @param {string} type [图片模式，'array'和 'sprite'模式，array是图片对象数组，sprite是基于宽度扩展的单张雪碧图]
 * @param {Array} imgs [图片帧对象数组或单图，对应不同模式]
 ...
 */
  ...
  /**
   * 初始化
   * @param {CanvasKeyFrames} self CanvasKeyFrames类实例
   */
  function init (self) {
    createCanvas(self);
    if (self.mode === 'array') {
  ...
  CanvasKeyFrames.prototype = {
     ...
     * @param {int} n 帧率
     */
    setSpeed: function (n, callback) {
      this.options.fps = n;
      this.isReset = false;
      this.fromTo(this.count + 1, this.recordTo, this.recordInf, callback);
    },
    /**
     * 跳到某一帧
     * @param {int} n 帧序列
    ...
    fromTo: function (from, to, loop, callback) {
      ...
      var timeFn = function timeFn () {
        if (self.ispause || self.state == 'stop') {
          return;
        }
        //当总量计数器达到帧总量的时候退出
     ...
     * @param {Function} callback [播放结束后回调函数]
     */
