...
namespace Com.Colin.Lib
{
    ...
    public class StringHelper
    {
        ...
        /// <param name="length">输出字串长度</param>
        /// <param name="w">占位字符</param>
        /// <param name="mode">占位符增加在原字串左边或者右边</param>
        /// <returns></returns>
        public static string getStrOfFixLen(string source,int length, char w, StrAddMode mode)
        {
            if(mode==StrAddMode.Left)
            {
                return source.PadLeft(length, w);
            }
            else
            {
                return source.PadRight(length, w);
            }
        }
    }
}
