...
namespace ClassLibrary1
{
    public sealed class SQLHelper
    {
        ...
        private static readonly string connStr = "server='" + server + "';database='" + database + "';uid='" + uid + "';pwd='" + pwd + "';Max Pool Size=100000;Min Pool Size=0;Connection Lifetime=0;packet size=32767;Connection Reset=false; async=true";
        #endregion
        ...
        {
            return ExecuteNonQuery(connStr, CommandType.StoredProcedure, commandText, commandParameters);
        }
        /// <summary>
        /// 执行指定数据库连接对象的命令
        /// </summary>
        ...
        /// <param name="commandType">命令类型 (存储过程,命令文本或其它,已统一加载)</param>
        /// <param name="commandText">存储过程名称或T-SQL语句</param>
        /// <returns>返回一个包含结果集的DataSet</returns>
        public static DataSet ExecuteDataset(string commandText, params SqlParameter[] commandParameters)
        {
            return ExecuteDataset(connStr, CommandType.StoredProcedure, commandText, commandParameters);
        }
        /// <summary>
        /// 执行指定数据库连接字符串的命令,返回DataSet.
        /// </summary>
        ...
        {
            return ExecuteReader(connStr, CommandType.StoredProcedure, commandText, commandParameters);
        ...
        public static object ExecuteScalar(string commandText, params SqlParameter[] commandParameters)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connStr))
            {
                ...
                // 调用指定数据库连接字符串重载方法.
                return ExecuteScalar(connection, CommandType.StoredProcedure, commandText, commandParameters);
            }
        }
        ...
        public static SqlCommand CreateCommand(string spName, SqlParameter[] parameterValues)
        {
        ...
        /// <param name="sourceColumns">源表的列名称数组</param>
        /// <returns>返回SqlCommand命令</returns>
        public static SqlCommand CreateCommand(string spName, params string[] sourceColumns)
        {
            SqlConnection connection = new SqlConnection(connStr);
            connection.Open();
            return CreateCommand(spName, sourceColumns);
        }
