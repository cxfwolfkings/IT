...
/**
 * namespace 这个东西是 PHP 在 5.3.0 以后的版本开始支持命名空间！
 * 命名空间一个最明确的目的就是解决重名问题
 * @author   Charles
 * @date     2015-12-6 下午7:46:45
 * @version  A.1
 */
namespace Think;
/**
...
class Think {
    ...
    static public function start() {
      /**
       * 将AUTOLOAD函数注册到SPL __autoload函数队列中。如果该队列中的函数尚未激活，则激活它们。
       */ 
      spl_autoload_register('Think\Think::autoload');      
      // 注册错误和异常处理函数，在脚本执行完毕或者调用exit()之后运行
      register_shutdown_function('Think\Think::fatalError');
      // 设置一个用户的函数appError来处理脚本中出现的错误。
      set_error_handler('Think\Think::appError');
      // 设置默认的异常处理程序，用于没有用 try/catch 块来捕获的异常。 在 appException调用后异常会中止。
    ...
    public static function autoload($class) {
        ...
        }elseif(false !== strpos($class,'\\')){//只有返回false整个语句才会得到 false，其他全部得到true，就算返回0也是得到true
 			/**
