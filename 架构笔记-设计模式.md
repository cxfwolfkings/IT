...
![x](./Resources/design_pattern_24.png)
...
![x](./Resources/design_pattern_25.png)
...
![x](./Resources/design_pattern_26.png)
...
![x](./Resources/design_pattern_27.png)
...
![x](./Resources/design_pattern_28.png)

接着上面举的画图程序的例子，先来看看在添加绘制圆的需求前的类结构：

![x](./Resources/design_pattern_29.png)

添加了圆的绘制以后的类结构：

![x](./Resources/design_pattern_30.png)
...
![x](./Resources/design_pattern_31.png)
...
![x](./Resources/design_pattern_32.png)
...
![x](./Resources/design_pattern_33.png)
...
![x](./Resources/design_pattern_34.png)
...
![x](./Resources/design_pattern_35.png)
...
![x](./Resources/design_pattern_36.png)
...
![x](./Resources/design_pattern_37.png)
...
![x](./Resources/design_pattern_38.png)
...
代理模式(Proxy Pattem) 是一个使用率非常高的模式
...
![x](./Resources/design_pattern_39.png)

我们先看一下类图中的三个角色的定义：

n Subject抽象主题角色：抽象主题类可以是抽象类也可以是接口，是一个最普通的业务类型定义，无特殊要求。

n RealSubject具体主题角色：也叫做被委托角色、被代理角色。它才是冤大头，是业务逻辑的具体执行者。

n Proxy代理主题角色：也叫做委托类、代理类。它负责对真实角色的应用，把所有抽象主题类定义的方放限制委托给真实主题角色实现，并且在真实主题角色处理完毕前后做预处理和善后处理工作。
...
n 职责清晰：真实的角色就是实现实际的业务逻辑，不用关心其他非本职责的事务，通过后期的代理完成一件事务，附带的结果就是编程简洁清晰。

n 高扩展性：具体主题角色是随时都会发生变化的，只要它实现了接口，甭管它如何变化，都逃不脱如来佛的手掌（接口），那我们的代理类完全就可以在不做任何修改的情况下使用。

n 智能化：Struts是如何把表单元素映射到对象上的示例
...
![x](./Resources/design_pattern_40.png)
...
![x](./Resources/design_pattern_41.png)
...
![x](./Resources/design_pattern_42.png)
...
![x](./Resources/design_pattern_43.png)
...
通用动态代理模型，很简单，两条独立发展的线路。动态代理实现代理的职责，业务逻辑Subject实现相关的逻辑功能，两者之间没有必然的相互摘合的关系。通知Advice从另一个切面切人，最终在高层模块也就是Client进行耦合，完成逻辑的封装任务。

![x](./Resources/design_pattern_44.png)

动态代理调用过程示意图

![x](./Resources/design_pattern_45.png)

动态代理的主要意图就是解决我们常说的“审计”问题，也就是横切面编程，在不改变我们已有代码结构的情况下增强或控制对象的行为。

要实现，动态代理的首要条件是：被代理类必须实现一个接口，回想一下前面的分析吧。当然了，现在也有很多技术如CGLIB可以实现，不需要接口也可以实现，动态代理的方式。
